...
// Simple pool of note block click sounds (simulated with a short blip or using external URL)
// Using a generic click sound for better performance/reliability without massive assets
const playTypingSound = () => {
  const audio = new Audio('https://www.myinstants.com/media/sounds/minecraft_click.mp3');
  audio.volume = 0.2;
  audio.playbackRate = 1.5 + Math.random() * 0.5; // Random pitch
  audio.play().catch(() => {});
};
...
export const InputOnly: React.FC<InputBaseProps> = ({ placeholder, value, onChange, required, type = "text", min, max, maxLength, showSkinPreview }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.value.length > value.length) {
      // Only play sound on adding characters
      playTypingSound();
    }
    onChange(e);
  };
